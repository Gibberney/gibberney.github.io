...
What it does:
  ...
  3. Calls Claude API to write a full article in your voice
...
Your output must be a single valid JSON object with exactly these fields:
  "title"      : A catchy, personal-sounding article title
...
def generate_article(prompt_text):
    message = claude.messages.create(
        ...
        messages=[{"role": "user", "content": f"Write an article about: {prompt_text}"}],
    )
...
<head>
  ...
  <style>
    body {{ font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background-color: #fdfdfd; color: #222; max-width: 750px; margin: 60px auto; padding: 0 20px; line-height: 1.6; }}
    header {{ display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #ccc; padding-bottom: 10px; margin-bottom: 40px; }}
    header a {{ color: #41414a; text-decoration: none; font-weight: bold; }}
    ...
    .ai-note  {{ margin-top: 60px; padding-top: 20px; border-top: 1px solid #eee; font-size: 0.82em; color: #aaa; font-style: italic; }}
...
def update_index_html(index_html, title, description, filename, pub_date_str):
    ...
    if "<!-- AUTO_BLOG_INSERT -->" not in index_html:
        raise ValueError("index.html is missing the <!-- AUTO_BLOG_INSERT --> marker.")
...
def main():
    ...
    prompt_entry = next(
        (a for a in prompts_data["articles"] if not a.get("used", False)), None
    )
    if not prompt_entry:
        print("No unused prompts remaining. Add more to prompts.json!")
        return
    ...
    github_put_file("index.html", updated_index, f"Add blog link: {article['title']}", sha=index_sha)

    prompt_entry["used"]           = True
    prompt_entry["published_date"] = today.isoformat()
    prompt_entry["article_file"]   = file_path
    github_put_file(PROMPTS_FILE_PATH, json.dumps(prompts_data, indent=2), "Mark prompt as used", sha=prompts_sha)

    print(f"Done! Published: https://gibberney.github.io/{file_path}")
